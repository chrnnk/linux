#!/bin/env python3
#
# https://github.com/ElectronicsWizardry/linuxDailyDriveWrites
#
# mkdir ~/drives
# touch ~/drives/drive-writes-daily.sh
# nano ~/drives/drive-writes-daily.sh
# chmod +x ~/drives/drive-writes-daily.sh
# ~/drives/drive-writes-daily.sh > ~/drives/`date +%Y-%m-%d`.txt
# crontab -e
# 0 12 * * * ~/drives/drive-writes-daily.sh > ~/drives/`date +%Y-%m-%d`.txt
